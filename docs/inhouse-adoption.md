...
4. **정적 자산과 게시 위치.** 현재 Mermaid 기본 주소는 공개 CDN입니다. 사내 자산 경로로 바꾸지 않으면 그림이 뜨지 않습니다.
...
Mermaid를 사내 경로에서 불러오도록 지정합니다. 엔드포인트 설정만으로 외부 통신이 차단되는 것은 아니므로, 실행 환경의 통신 경계도 함께 확인해야 합니다.
...
- **빌드 시스템이 다릅니다.** libcamera는 Meson, 사내 HAL은 NDK/AOSP입니다. compile DB의 인자 형식과 생성 헤더 위치가 다릅니다.
