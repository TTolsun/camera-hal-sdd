...
- 선언에 붙지 않은 문서 주석 블록(`\class`, `\struct` + `\brief`)을 따로 읽어 비어 있는 클래스 설명을 채웁니다. libclang 은 선언 바로 앞의 주석만 주므로, 클래스 설명을 구현 파일에 따로 적는 코드베이스에서는 설명이 통째로 빠집니다. 이름이 한정 이름으로 맞거나 짧은 이름 후보가 하나일 때, 또는 같은 디렉터리에 선언된 후보가 하나일 때만 붙입니다. 그래도 좁혀지지 않으면 비우고, 선언에 붙은 주석이 있으면 덮어쓰지 않습니다. libcamera 에서 설명 보유가 331/1005(32%)에서 546/1005(54%)로 올랐습니다.
...
- `kind: manual` 문서는 파일이 없으면 "아직 작성되지 않았습니다" 라고 적은 자리를 만들고 `needs-review` 로 표시합니다. 사람이 채우면 이후 생성에서 덮어쓰지 않습니다. 설정에 있는 문서로 건 링크가 깨지지 않게 하면서, 써야 할 문서를 문서 목록에 남겨 둡니다.
...
빌드 파일 감시는 그 섹션이 다루는 영역으로 좁힙니다. 전역 `**/meson.build` 를 두면 관계 없는 디렉터리의 빌드 파일 변경이 모든 섹션을 끌고 옵니다. libcamera `0a881d26`→`279d355e` 로 실측했을 때, IPU3 알고리즘의 `meson.build` 하나 때문에 `camera-model` 과 `pipeline-handler` 까지 재생성 대상이 되어 7개 섹션이 전부 선택됐습니다. 좁힌 뒤에는 5개가 선택되고 나머지는 이월했습니다. 기능 추가 커밋은 거의 항상 빌드 파일을 건드리므로, 좁히지 않으면 증분 생성이 전체 재생성이 됩니다.
