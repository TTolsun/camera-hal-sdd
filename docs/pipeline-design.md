...
- 선언에 붙지 않은 문서 주석 블록(`\class`, `\struct` + `rief`)을 따로 읽어 비어 있는 클래스 설명을 채웁니다. libclang 은 선언 바로 앞의 주석만 주므로, 클래스 설명을 구현 파일에 따로 적는 코드베이스에서는 설명이 통째로 빠집니다. 이름이 한정 이름으로 맞거나 짧은 이름 후보가 하나일 때, 또는 같은 디렉터리에 선언된 후보가 하나일 때만 붙입니다. 그래도 좁혀지지 않으면 비우고, 선언에 붙은 주석이 있으면 덮어쓰지 않습니다. libcamera 에서 설명 보유가 331/1005(32%)에서 546/1005(54%)로 올랐습니다.
