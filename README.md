...
| 인용 검사 | 인용 토큰이 facts의 위치 집합에 있는지 검사합니다. 문장 형태(종결어미, 대화체, 프롬프트 누설)는 `manuscript.py` 린트가 함께 검사합니다. 문장의 의미가 참인지 보장하지 않습니다. |
| 사이트 구성 | 탐색·목차·검색·그림 확대를 제공하고 내부 링크와 산출물 해시를 검사합니다. |
| 게시 | 검토 후 별도 배포 절차로 진행합니다. upstream 감시와 Git 자동 게시 기능은 없습니다. |

`ok`는 인용 검사 결과입니다. `mapped`는 설정상 문서 범위 연결 결과입니다. 둘 다 사람의 승인이나 설계 문서의 충분성을 뜻하지 않습니다.
...
남은 주요 과제는 설명의 의미 검증, 요청·버퍼·종료·동시성 등 설계 내용 보강, 변경되지 않은 원고의 근거 재검증, 사내 HAL에서의 실제 검증입니다. 가상 호출·스레드·수명 관계는 정적 추출만으로 완전하게 설명할 수 없으며 코드 검토와 실행 근거가 필요합니다.
